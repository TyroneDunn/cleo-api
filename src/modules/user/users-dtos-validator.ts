...
export const validateGetUsersDTO = async (user: User, dto: GetUsersDTO): Promise<ValidationResult> => {
    ...
    if (dto.isAdmin) {
        if (dto.isAdmin.toLowerCase() !== 'true' && dto.isAdmin.toLowerCase() !== 'false')
            return {outcome: false, error: new BadRequestError('Invalid query. isAdmin must be' +
                    ' true or false')};
    }
    if (dto.status) {
        if (dto.status !== 'active' && dto.status !== 'inactive' && dto.status !== 'suspended')
            return {outcome: false, error: new BadRequestError('Invalid query. Status must be' +
...
export const validateUpdateUserDTO = async (user: User, dto: UpdateUserDTO): Promise<ValidationResult> => {
    ...
    if (user.username !== dto.username &&
        ...
        return {outcome: false, error: new ForbiddenError('Insufficient permissions.')};
    if (!(await USERS_REPOSITORY.exists(dto.username)))
        return {outcome: false, error: new NotFoundError(`User ${dto.username} not found.`)};
    if (dto.newUsername) {
        if (dto.newUsername.length < 3)
            return {outcome: false, error: new BadRequestError('Username must be at least 3' +
                    ' characters.')};
        if (await USERS_REPOSITORY.exists(dto.newUsername))
            return {outcome: false, error: new ConflictError('Username already taken.')};
    }
    if (dto.newPassword) {
        if (dto.newPassword.length < 8)
            return {outcome: false, error: new BadRequestError('Password must be at least 8' +
                    ' characters.')};
    }
    if (dto.newIsAdmin) {
        if (!(await USERS_REPOSITORY.isAdmin(user.username)))
